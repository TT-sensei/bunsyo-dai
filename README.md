...
- 7問を最後まで終えると、edu-assetsの150種類から未獲得のコレクションバッジを1つ発見する
- 入口にコレクション一覧を置き、見つけたバッジを保存する
...
collection.js には edu-assets のコレクション10シリーズ、150種類を登録しています。1回の旅をゴールするたびに未獲得のバッジから1つをランダムに選ぶため、同じバッジが続けて出ず、少しずつコレクションが増えていきます。
...
- collection.js：edu-assetsの150種類のコレクションバッジ一覧
- common.js：edu-kit接続、保存、互換移行、NAVI・音の設定
- styles.css：入口・問題・結果の共通レイアウト

## 保存

新しい保存領域は edu:bunsyo-dai: namespaceです。コレクションは `collections` 配列に獲得済みバッジIDとして保存します。
